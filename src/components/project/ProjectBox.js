...
const ProjecBox = ({ title, details, tags, image, href }) => {
  return (
    <div className="relative flex-1 md:basis-1/2 transition-all duration-300 bg-secondary-dark p-6 group-hover:md:basis-1/4 hover:md:basis-3/4  md:h-100 lg:h-80 2xl:h-70">
      <div className="flex flex-col group/box w-full h-full ">
        <h3 className="font-clash-display text-2xl text-main-text-dark font-semibold pb-10 pt-2 tracking-wide">
          {title}
        </h3>
        <div className="group-hover/box:max-w-100 2xl:group-hover/box:max-w-none">
          <ProjectCardInfoTech
            tags={tags}
            className="group-hover/box:bg-hover"
          />
          <div
            className="overflow-hidden transition-opacity duration-300
            group-hover/box:md:opacity-100 group-hover/box:opacity-100 group-hover:opacity-0"
          >
            <p className="text-sm md:text-base text-main-text-dark py-5 max-w-100">
              {details}
            </p>
